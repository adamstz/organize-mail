    ...
    def _detect_query_type(self, question: str) -> str:
        ...
        try:
            ...
            # Map common response words to actual types
                ...
                logger.debug(
                    "[QUERY DETECTION] LLM returned unexpected value: '%s', defaulting to semantic",
                    classification,
                )
                detected_type = 'semantic'

            logger.debug("[QUERY DETECTION] LLM classified query as: %s", detected_type)
            return detected_type

        except Exception as e:
            logger.debug("[QUERY DETECTION] LLM classification failed (%s), using fallback heuristic", e)
...
Just ask me anything about your emails!"""
    ...
    def _handle_aggregation_query(self, question: str) -> Dict:
        ...
        if 'how many' in question_lower and not any(word in question_lower for word in ['total', 'per day', 'unread']):
            ...
            extraction_prompt = (
                "You are extracting a company or sender name from a user's question. "
            ...
            try:
                ...
                logger.debug("[AGGREGATION] SQL query params: %s", params)
                ...
                logger.debug("[AGGREGATION] Failed to extract topic: %s, falling back to standard aggregation", e)
    ...
    def _handle_search_by_sender(self, question: str, limit: int) -> Dict:
        ...
        extraction_prompt = (
            "Extract the sender name or email address from this query. "
...
Keywords:"""
        ...
        logger.debug("[FILTERED TEMPORAL] Executing SQL query with %d keyword(s)", len(keywords))
        cur.execute(sql, params)
        rows = cur.fetchall()
        ...
        logger.debug("[FILTERED TEMPORAL] Found %d matching emails", len(rows))
        ...
        logger.debug("[FILTERED TEMPORAL] Query completed successfully")
    ...
    def _handle_semantic_query(self, question: str, limit: int, threshold: float) -> Dict:
        ...
        if is_counting_query:
            ...
            logger.debug(
                "[SEMANTIC QUERY] Counting query detected - increased limit from %s to %s, lowered threshold from %s to %s",
                original_limit,
                limit,
                original_threshold,
                threshold,
        ...
        logger.debug("[SEMANTIC QUERY] Step 1: Generating embedding for question")
        try:
            question_embedding = self.embedder.embed_text(question)
            logger.debug("[SEMANTIC QUERY] Embedding generated successfully")
        except Exception as e:
        ...
        logger.debug("[SEMANTIC QUERY] Step 2: Searching for similar emails (limit=%s, threshold=%s)", limit, threshold)
        try:
            ...
            logger.debug("[SEMANTIC QUERY] Found %d similar emails", len(similar_emails))
        except Exception as e:
        ...
        logger.debug("[SEMANTIC QUERY] Similarity scores:")
        for i, (email, score) in enumerate(similar_emails[:5]):  # Show top 5
            logger.debug(
                "[SEMANTIC QUERY]   %d. Score: %.3f - Subject: '%s...' From: %s",
                i + 1,
                score,
                email.subject[:50],
                email.from_,
        ...
        logger.debug("[SEMANTIC QUERY] Step 3: Building context from %d emails", len(similar_emails))
        try:
            context = self.context_builder.build_context(similar_emails)
            context_length = len(context)
            logger.debug("[SEMANTIC QUERY] Context built successfully (%d characters)", context_length)
            logger.debug("[SEMANTIC QUERY] Context preview: %s...", context[:200])
        except Exception as e:
        ...
        logger.debug("[SEMANTIC QUERY] Step 4: Generating answer with LLM")
        try:
            answer = self._generate_answer(question, context)
            answer_length = len(answer)
            logger.debug("[SEMANTIC QUERY] Answer generated successfully (%d characters)", answer_length)
            logger.debug("[SEMANTIC QUERY] Answer preview: %s...", answer[:200])
        except Exception as e:
        ...
        logger.debug("[SEMANTIC QUERY] Step 5: Formatting sources")
        sources = [
            {
                'message_id': msg.id,
                'subject': msg.subject,
    ...
    def _generate_answer(self, question: str, context: str) -> str:
        ...
        # Build RAG prompt
        prompt = (
            "You are an email assistant. I have retrieved emails from the user's mailbox "
            "and YOU MUST analyze them.\n\n"
    ...
    def _generate_answer_classification(self, question: str, context: str, messages: List, total_count: int, label: str) -> str:
        ...
        prompt = (
            ...
            f"The user has asked about emails with the classification label: \"{label}\"\n\n"
            f"TOTAL EMAILS WITH THIS LABEL: {total_count}\n\n"
            f"I am providing you with {len(messages)} sample emails (limited for context) "
            "from this category.\n\n"
            f"===== SAMPLE EMAILS WITH LABEL \"{label}\" =====\n\n"
            f"{context}\n\n"
            "===== USER QUESTION =====\n\n"
            f"{question}\n\n"
            "===== YOUR ANSWER =====\n\n"
            f"Based on the classification data, there are {total_count} emails labeled as \"{label}\". "
    ...
    def _generate_answer_temporal(self, question: str, context: str, messages: List) -> str:
        ...
        prompt = (
            "You are an email assistant with direct access to the user's email database.\n\n"
            "I am providing you with the user's actual emails from their database. "
            "You MUST analyze these emails to answer their question.\n\n"
            "IMPORTANT: You have full access to these emails - they are real emails from "
            "the user's mailbox. Analyze them and provide a helpful answer.\n\n"
    ...
    def _call_llm(self, prompt: str) -> str:
        ...
        if self.llm.provider == "ollama":
            ...
            logger.info("[LLM CALL] Using Ollama provider")
            ...
            logger.debug("[LLM CALL] Ollama request to %s, model: %s", host, self.llm.model)
            ...
            try:
                ...
                logger.debug("[LLM CALL] Sending request to Ollama...")
                with urllib.request.urlopen(req, timeout=300) as response:
                    result = json.load(response)
                    response_text = result.get("response", "Unable to generate answer")
                    elapsed_time = time.time() - start_time
                    logger.info("[LLM CALL] Ollama response received in %.2fs", elapsed_time)
                    logger.debug("[LLM CALL] Response length: %d characters", len(response_text))
                    logger.debug("[LLM CALL] Response preview: %s...", response_text[:200])
                    return response_text
            except Exception as e:
                logger.debug("[LLM CALL] Ollama request failed: %s", e)
                raise
            ...
            logger.info("[LLM CALL] Using OpenAI provider")
            try:
                start_time = time.time()
                ...
                elapsed_time = time.time() - start_time
                logger.info("[LLM CALL] OpenAI response received in %.2fs", elapsed_time)
                logger.debug("[LLM CALL] Response length: %d characters", len(response_text))
                logger.debug("[LLM CALL] Response preview: %s...", response_text[:200])
                return response_text
            except Exception as e:
                logger.debug("[LLM CALL] OpenAI request failed: %s", e)
                raise
            ...
            logger.info("[LLM CALL] Using Anthropic provider")
            try:
                start_time = time.time()
                ...
                elapsed_time = time.time() - start_time
                logger.info("[LLM CALL] Anthropic response received in %.2fs", elapsed_time)
                logger.debug("[LLM CALL] Response length: %d characters", len(response_text))
                logger.debug("[LLM CALL] Response preview: %s...", response_text[:200])
                return response_text
            except Exception as e:
                logger.debug("[LLM CALL] Anthropic request failed: %s", e)
                raise

        else:
            # Fallback for other providers
            error_msg = f"RAG queries require Ollama, OpenAI, or Anthropic. Current provider: {self.llm.provider}"
            logger.debug("[LLM CALL] Unsupported provider: %s", error_msg)
            return error_msg
